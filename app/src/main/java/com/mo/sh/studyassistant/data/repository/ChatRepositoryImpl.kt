    ...
    override suspend fun sendPrompt(
        ...
        try {
            val response = api.generateText(
                systemInstruction = getSectionSystemPrompt(section),
            ...
            e.printStackTrace()
            messagesDao.deleteChat(cId)
            NetworkResult.Error.Unknown(e.message ?: App.getString(R.string.unknown_error))
        }
    }
    ...
    override suspend fun sendMessage(
        ...
        try {
            val response = api.generateMessage(
                systemInstruction = getSectionSystemPrompt(MessageSection.Tutor),
                history = chat?.messages?.reversed()?.map {
                    content(
